...
login_matin_data = {"username": "matin", "password": "1234"}
...
class APITest:
    ...
    def add_admin(self):
        r = requests.post(f"{BASE}/user", json=add_admin_data)
        print("add_admin:", r.json())

    def add_matin(self):
        r = requests.post(f"{BASE}/user", json=add_matin_data)
        print("add_matin:", r.json())
    ...
    def login_matin(self):
        r = requests.post(f"{BASE}/auth/login", json=login_matin_data)
        print("login_matin:", r.json())
        self.__class__.matin_token = r.json().get("data", {}).get("token", "")

    def add_author(self):
        for a in authors:
            r = requests.post(f"{BASE}/author", json=a, headers=make_header(self.token))
    ...
    def add_credit(self):
        r = requests.post(f"{BASE}/credit", json=addcredit, headers=make_header(self.matin_token))
        print("add_credit:", r.json())

    def add_cart(self):
        r1 = requests.post(f"{BASE}/cart/add", json=cart1, headers=make_header(self.matin_token))
        print("add_cart1:", r1.json())
        r2 = requests.post(f"{BASE}/cart/add", json=cart2, headers=make_header(self.matin_token))
        print("add_cart2:", r2.json())

    def borrow_book(self):
        r = requests.post(f"{BASE}/cart/borrow", json=borrow, headers=make_header(self.matin_token))
        print("borrow:", r.json())

    def list_cart(self):
        r = requests.get(f"{BASE}/cart/list", headers=make_header(self.matin_token))
        print("cart_list:", r.json())

    def purchase(self):
        r = requests.post(f"{BASE}/cart/purchase", headers=make_header(self.matin_token))
        print("purchase:", r.json())

    def history(self):
        r = requests.get(f"{BASE}/purchase-history", headers=make_header(self.matin_token))
        print("history:", r.json())

    def add_review(self):
        r = requests.post(f"{BASE}/books/peanuts/review", json=add_review, headers=make_header(self.matin_token))
